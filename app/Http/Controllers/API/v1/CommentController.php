    ...
    public function getVillaComments($id)
    {
        ...
        foreach ($comments as $key => $value) {
            if ($value->parent_id == 0) {
                ...
                $answersId[$key] = $value->parent_id;
            }
        }

        foreach ($main as $mainKey => $mainValue) {
            foreach ($answers as $answerKey => $answerValue) {
                if ($mainValue->id == $answerValue->parent_id) {
                    $mainValue['answer'] = [
                        'text' => $answerValue->text,
                        'created_at' => Verta::instance($answerValue->created_at)->format('%Y ,%B %d')
                    ];
                }
            }
        }
        ...
        return new VillaComments($main, $scores);
    }
